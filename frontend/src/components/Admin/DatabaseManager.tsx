...
const mockMedicaments = [
  {
    ...
    diagnostics: ["Infection respiratoire", "Otite"],
  },
  {
    id: 2,
    nom: "Doliprane",
    ...
    diagnostics: ["Fièvre", "Douleur légère à modérée"],
  },
  {
    id: 3,
    nom: "Kardegic",
    forme: "Sachet",
    dosage: "75mg",
    molecule: "Acide acétylsalicylique",
    diagnostics: ["Prévention cardiovasculaire"],
  },
]

export function DatabaseManager({ isAdmin = false }: { isAdmin?: boolean }) {
  const [activeTab, setActiveTab] = useState<
    "medicaments" | "molecules" | "diagnostics"
  >("medicaments")
        ...
        <div className="overflow-x-auto">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {med.diagnostics.map((diag, i) => (
                          <Badge
                            key={i}
                            variant="secondary"
                            className="bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 border-0 font-normal"
                          >
                            {diag}
                          </Badge>
                        ))}
          ...
          {activeTab !== "medicaments" && (
            <div className="flex flex-col items-center justify-center p-12 text-muted-foreground">
              La vue pour {activeTab} utilise la même logique et sera
